...
const Join = () => {
    ...
    const usernameCheck=useCallback(async ()=>{
        try {
            ...
            const response=await axios.post('http://211.188.50.151:9090/members/username-check',{
                username:joinForm.username
            });
            if (response.data.item.usernameCheckMsg==='invalid username'){
                alert('중복된 아이디 입니다. 다른 아이디로 변경해 주세요')
                document.querySelector("#username").focus();
                return;
            }
            else{
                if (window.confirm(`${joinForm.username}은 사용 가능한 아이디 입니다. 사용하시겠습니까?`)){
    ...
    const nicknameCheck=useCallback(async ()=>{
        try {
            ...
            const response=await axios.post('http://211.188.50.151:9090/members/nickname-check',{
                nickname:joinForm.nickname
            });
            if (response.data.item.nicknameCheckMsg==='invalid username'){
                alert('중복된 닉네임 입니다. 다른 닉네임으로 변경해 주세요')
                document.querySelector("#nickname").focus();
                return;
            }
            else{
                if (window.confirm(`${joinForm.nickname}은 사용 가능한 닉네임 입니다. 사용하시겠습니까?`)){
    ...
    },[joinForm,usernameChk,passwordValidate,passwordChk,nicknameChk,dispatch])
  return (
        ...
        <form onSubmit={submitHandler}>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} textAlign='right'>
                    ...
                    ></TextField>
                    <Button name='username-check-btn' id='username-check-btn' color='primary' onClick={usernameCheck}>
                        중복확인
                    </Button>
                </Grid>
                <Grid item xs={12}>
                ...
                <Grid item xs={12} textAlign='right'>
                    <TextField
                        ...
                        onChange={changeTextField}
                    ></TextField>
                    <Button name='nickname-check-btn' id='nickname-check-btn' color='primary' onClick={nicknameCheck}>
                        중복확인
                    </Button>
                </Grid>
                <Grid item xs={12}>
